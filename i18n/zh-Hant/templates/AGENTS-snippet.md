...
1. **開工**：先讀 `collab_log/INDEX.md` —— 規則 +「## 🔴 現在進行中」區塊 + 最近幾天摘要；把「現在進行中」每一條對照 `git status --short` / 近期 `git log`，過期的當場修正；再讀今天的日檔 `collab_log/<今天日期>.md`（不存在就用範本新建）。
2. **回填昨天總結**：用 grep 找 `collab_log/INDEX.md` 裡總結欄還是 ⏳ 的舊日子；讀完該日所有 entry 後補寫其 `## 當日總結`（3–6 行），並把 INDEX 索引表該列改成 ✅。
3. **收工**（完成一個工作單元後）：在今天日檔 `# Entries` 區**最上方** append 一條 entry（did / verify / files / handoff）；同時**覆寫** INDEX 的「## 🔴 現在進行中」區塊，反映最新狀態（只放 open threads，不放 git 已記錄的工作區狀態）。只寫 entry 不更新區塊，不算收工。
