...
>
> **這裡只放 open threads**——沒做完的事、待決策、已知地雷。**絕不要**記 git 已經追蹤的狀態（未提交檔案、WIP diff）：任何人一 commit 它就過期。這個區塊跟 git 說法不一致時，以 git 為準——修正區塊。
...
2. **核對區塊與現實**：把「現在進行中」每一條對照 `git status --short` 與近期 `git log`；有不再成立的（例如「X 未提交」但工作區乾淨），動工前先修正區塊。
3. **讀「今天」的日期檔** `collab_log/<今天日期>.md`；不存在就用下面的範本新建（含空的 `## 當日總結` 區塊）。
4. **補寫昨天的總結（若還沒寫）**：用 `Grep pattern="⏳" path=collab_log/INDEX.md` 直接定位總結欄還是 ⏳ 的舊日子（不要 Read 整檔再人工掃），讀完它所有 entry，在該檔的 `## 當日總結` 寫 3–6 行（今天整體做了什麼、還開著什麼），並回來更新下方索引表那一行為 ✅。

## 收工流程（完成一個工作單元後）
1. 在**今天**的日期檔，**`# Entries` 區、最新在上**，append 一條 entry（用下方範本）。
2. **覆寫上面的「🔴 現在進行中」區塊**，反映最新的 open threads（做完的拿掉、新開的加上）。只寫 entry 不更新這個區塊，不算完成收工。
3. 不刪歷史、不改別人的舊 entry。
