...
兩個 AI coding agent —— **Claude**(Anthropic, Claude Code)與 **Codex**(OpenAI, Codex CLI)—— 在同一個 repo 上工作。它們不共享記憶,唯一的共同記憶是專案根目錄的 `collab_log/` 資料夾:一份每日交接日誌,**兩個 agent 開工時都讀、收工時都寫**。
